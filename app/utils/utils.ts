...
export function i0(signature: string): bigint {
  ...
  const bytes = hexToBytes(cleanSig);
 ...
 */
export async function deriveKeysFromUser(signature: string): Promise<{
  privateKey: bigint;
 ...
 */
export async function getDecryptedBalance(privateKey: bigint, amountPCTs: { pct: bigint[] }[], balancePCT: bigint[], encryptedBalance: bigint[][]): Promise<bigint> {
